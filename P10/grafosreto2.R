...
prob <-as.data.frame(read.csv("r2pfinal.csv"))
ambos<- data.frame()
ambos<-rbind(noprob,prob)
colnames(ambos) <- c("X","Generacion","Valor","Diferencia","Tipo")
ambos$Generacion <- as.factor(ambos$Generacion)

png(paste("Reto2.png", sep=""), width=700, height=700)
ggplot(data=ambos,aes(x=Generacion,y=Diferencia,fill=Tipo))+geom_boxplot() +xlab("Poblaci\u{F3}n")+ylab("GAP")#stat_summary(fun.y=mean,geom="smooth",aes(group=Tipo,col=Tipo))
graphics.off()
#????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
#Prueba Estad??stica
...
#title(main="Tendencia", col.main="red", font.main=4)
