...
for(va in 1:4){
  ...
  stopCluster(cluster)
  png("p10prob.png", width=600, height=300)
  ...
  tipos <- rep("Probabilidad",40)
  valobj <-cbind(valobj,tipos)
colnames(valobj) <- c("Población","Valor","Diferencia","Tipo")


#print(valobj)
write.csv(valobj, file="r1probfinal.csv")
