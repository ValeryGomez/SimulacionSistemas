...
for (iter in 1:tmax) {
  ...
  tiempsec <- rbind(tiempsec,c(init,tie))
}
}

tipos <- rep("Secuencial",tmax*length(valss))
tiempsec <-cbind(tiempsec,tipos)
colnames(tiempsec) <- c("Población","Tiempos","Tipo")
write.csv(tiempsec, file="TiemposNoP.csv")
