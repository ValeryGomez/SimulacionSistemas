...
frente <- subset(val, no.dom) # solamente las no dominadas
ln <- length(no.dom)
x<-round(runif(2,1,ln))
m.x <- c(val[x[1],1], val[mejor2,1])
m.y <- c(val[x[2],2], val[mejor2,2])
da <- data.frame(m.x,m.y)
png("p11_frenteMor.png")
plot(val[,1], val[,2], xlab=paste(xl, "mejor con cuadro azul"),
     ylab=paste(yl,"mejor con bolita naranja"),
     main="Ejemplo bidimensional")
points(frente[,1], frente[,2], col="green", pch=16, cex=1.5)
points(da, col="red",pch=32, cex=1.5)
graphics.off()
frente <- subset(val, no.dom) # solamente las no dominadas
png("p11_frente.png")
plot(val[x[1],1], val[x[1],2], xlab=paste(xl, "mejor con cuadro azul"),
     ylab=paste(yl,"mejor con bolita naranja"),
     main="Ejemplo bidimensional")
points(frente[,1], frente[,2], col="purple", pch=16, cex=1.5)
points(da[,1],da[,2], col="purple",pch=32,cex=2)
graphics.off()
