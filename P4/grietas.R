...
colnames(datos)= c("Largo","Dimensi??n", "Semilla")
write.csv(datos,file="datos.csv")

datos12= datos[which(datos$Dimensi??n==12),]
png(paste("12.png",sep=""))
boxplot(data=datos12,use.cols=FALSE, aes(x=factor(Semilla),y=Largo))+labs(x="N??mero de region",y="Largos")
graphics.off()

datos20= datos[which(datos$Dimensi??n==20),]
png(paste("20.png",sep=""))
boxplot(data=datos12,use.cols=FALSE, aes(x=factor(Semilla),y=Largo))+labs(x="N??mero de region",y="Largos")
graphics.off()

datos50= datos[which(datos$Dimensi??n==50),]
png(paste("50.png",sep=""))
boxplot(data=datos12,use.cols=FALSE, aes(x=factor(Semilla),y=Largo))+labs(x="N??mero de region",y="Largos")
graphics.off()

datos70= datos[which(datos$Dimensi??n==70),]
png(paste("70.png",sep=""))
boxplot(data=datos12,use.cols=FALSE, aes(x=factor(Semilla),y=Largo))+labs(x="N??mero de region",y="Largos")
graphics.off()

datos100= datos[which(datos$Dimensi??n==100),]
png(paste("100.png",sep=""))
boxplot(data=datos12,use.cols=FALSE, aes(x=factor(Semilla),y=Largo))+labs(x="N??mero de region",y="Largos")
